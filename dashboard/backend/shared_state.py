...
def update_patient(stay_id: int, data: Dict[str, Any]) -> None:
    ...
    with _lock:
        ...
        patient = {
            ...
            # Labs (may come from separate update)
            ...
            'labs_age_minutes': data.get('labs_age_minutes', existing.get('labs_age_minutes', 999)),
...
def update_patient_labs(stay_id: int, labs_data: Dict[str, Any]) -> None:
    ...
    with _lock:
        if stay_id in _patients:
            ...
            _patients[stay_id]['labs_age_minutes'] = labs_data.get('labs_age_minutes', 0)
